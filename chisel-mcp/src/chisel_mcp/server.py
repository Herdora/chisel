...
@mcp.tool()
def profile_nvidia(target: str, gpu_type: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if target_path.exists() and not target_path.is_file():
            return {"success": False, "error": f"Target '{target}' is not a file."}
        ...
        if result.returncode == 0 and "Profiling completed successfully" in output:
            ...
            return {
                "success": True,
                "output_dir": results_match.group(1)
                if results_match
                else "chisel-results",
                "cost_estimate": float(cost_match.group(1)) if cost_match else 0.0,
                "profile_files": {"ncu": ncu_files, "nsys": nsys_files},
            ...
            if "No API token configured" in error_msg:
                ...
                return {"success": False, "error": error_msg or "Profiling failed"}

    except subprocess.TimeoutExpired:
        return {"success": False, "error": "Profiling timed out after 30 minutes."}
    except Exception as e:
        return {"success": False, "error": f"Profiling error: {str(e)}"}


@mcp.tool()
def profile_amd(target: str, pmc: Optional[str] = None) -> Dict[str, Any]:
    """
