...
def _run_hip_profiling(file_path: str, hipcc_flags: str, no_cleanup: bool, 
                      session_name: str, profiling_args: dict, counters: Optional[str] = None):
...
@click.option('--no-cleanup', is_flag=True, help='Keep connection to pod after execution')
def hip_trace(file_path: str, hipcc_flags: str, no_cleanup: bool):
    """Profile HIP kernels with basic runtime tracing."""
    _run_hip_profiling(file_path, hipcc_flags, no_cleanup, "runtime tracing", 
...
def hip_kernel(file_path: str, hipcc_flags: str, no_cleanup: bool):
    ...
    _run_hip_profiling(file_path, hipcc_flags, no_cleanup, "kernel analysis", 
...
def hip_system(file_path: str, hipcc_flags: str, no_cleanup: bool):
    ...
    _run_hip_profiling(file_path, hipcc_flags, no_cleanup, "system tracing", 
...
def hip_memory(file_path: str, hipcc_flags: str, no_cleanup: bool):
    ...
    _run_hip_profiling(file_path, hipcc_flags, no_cleanup, "memory profiling", 
                      {"include_hip_trace": False, "output_prefix": "rocprof_memory_trace"})


@main.command()
@click.argument('file_path', type=click.Path(exists=True))
@click.option('--counters', help='Comma-separated list of performance counters (e.g., SQ_WAVES,GRBM_COUNT)')
@click.option('--hipcc-flags', default='-O2', help='Additional hipcc compilation flags')
@click.option('--no-cleanup', is_flag=True, help='Keep connection to pod after execution')
def hip_counters(file_path: str, counters: str, hipcc_flags: str, no_cleanup: bool):
    ...
    _run_hip_profiling(file_path, hipcc_flags, no_cleanup, "counter collection", {}, counters)
